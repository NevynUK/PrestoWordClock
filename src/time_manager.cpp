...
void time_manager_get(int &hour, int &minute, int &second)
{
    ...
    uint64_t elapsed_s = absolute_time_diff_us(s_ntp_sync_time, get_absolute_time()) / 1000000ULL;

    uint64_t unix_secs = static_cast<uint64_t>(s_unix_epoch_secs) + elapsed_s + static_cast<int64_t>(UTC_OFFSET_SECONDS);
